...
const Generator = (props) => {
    ...
    return (
        <SectionWrapper id={'generate'} header={'Generate your Workout'} title={['It\'s', 'Workout', 'time']}>
                ...
                {Object.keys(WORKOUTS).map((type, typeIndex) => {
                    return (
                        ...
                        <button onClick={() => {
                            setMuscles([])
                            setPoison(type)
                        }} className={'bg-stone-950 border  duration-200 px-4 hover:border-red-600 py-3 rounded-lg ' + (type === poison ? ' border-red-800' : ' border-red-400')} key={typeIndex}>
                ...
                {Object.keys(SCHEMES).map((scheme, schemeIndex) => {
                    return (
                        ...
                        <button onClick={() => setGoal(scheme)}
                            className={'bg-stone-950 border  duration-200 px-4 hover:border-red-600 py-3 rounded-lg ' + (scheme === goal ? ' border-red-800' : ' border-red-400')}
                            key={schemeIndex}>
